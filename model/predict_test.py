from model import tokenizer, llm_model, df
from create_prompt import create_prompt_text_test
import pandas as pd

csv_file_path = r"C:\Users\Debajyoti\OneDrive\Desktop\project task-1\data\test_dataframe.csv"
...
def create_summary_all_test_df(df, example_indices_full, rationaleies, test_df):
    pred_rationaleies = []
    
    for i in range(len(test_df)):
        current_prompt = create_prompt_text_test(df, example_indices_full, i, rationaleies, test_df)
        inputs = tokenizer.encode(current_prompt, return_tensors='pt')  # Ensure proper encoding
...
pred_rationaleies = create_summary_all_test_df(df, example_indices_full, rationaleies, test_df)

test_df['Rationale'] = pred_rationaleies

test_df.to_csv(r"C:\Users\Debajyoti\OneDrive\Desktop\project task-1\data\test_with_rationale.csv", index=False)

print("Successfully completed processing and saved summaries.")
