...
def conference_label(row):
    ...
    row['conference_label'] = mapping.get(row['Conference'], None)  
    return row
...
df = df.apply(conference_label, axis=1)
texts = df['PDF'].tolist() 
labels = df['conference_label'].tolist()
...
model.save_weights(r"C:\Users\Debajyoti\OneDrive\Desktop\project task-1\model\bert_model_categorical_weights.weights.h5")
